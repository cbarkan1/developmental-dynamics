...
def find_Ns(Pop, S_th, D_th, living_indices=None):
    ...
    N_St = np.sum(np.logical_and(
        Pop[living_indices, 1] <= S_th, Pop[living_indices, 1] >= D_th))
...
def V(y1, y2, lambdaD):
    ...
    V1 = r_R1*Gamma_D1 + Pt1*(r_Rt1-r_R1)*Gamma_D1 \
        + GammaR1*(r_01-r_R1*Gamma_D1) \
        + Pt1*GammaR1*(r_t1+r_R1*Gamma_D1-r_01-r_Rt1*Gamma_D1) - k1*y1

    Pt2 = y2**n/(a+y2**n)
    GammaR2 = a/(a+y1**n)
    V2 = r_R2 + Pt2*(r_Rt2-r_R2) + GammaR2*(r_02-r_R2) \
        + Pt2*GammaR2*(r_t2+r_R2-r_02-r_Rt2) - k2*y2
