"""
Plots the Waddington Vector Fields in Figure 2B
"""
...
y1_range = np.linspace(-1, 41, 50)
y2_range = np.linspace(-1, 26, 50)
...
# Left panel (c_1 = 0)
c_1 = 0
F_A, F_B = model.F(y1_mesh, y2_mesh, c_1=c_1)
a.streamplot(y1_mesh, y2_mesh, F_A, F_B, density=stream_density)
Y1s = [[1.5, 2], [1.5, .1]]
for Y1 in Y1s:
...
c_1 = 30
F_A, F_B = model.F(y1_mesh, y2_mesh, c_1=c_1)
b.streamplot(y1_mesh, y2_mesh, F_A, F_B, density=stream_density)
...
c_1 = 50
F_A, F_B = model.F(y1_mesh, y2_mesh, c_1=c_1)
c.streamplot(y1_mesh, y2_mesh, F_A, F_B, density=stream_density)
...
zS, zD = parameters["zS"], parameters["zD"]
for ax in [a, b, c]:
    ax.plot([0, 40], [zS, zS], dashes=[6, 2], color='k', linewidth=1.4)
    ax.plot([0, 40], [zD, zD], dashes=[6, 2], color='k', linewidth=1.4)
    ax.set_xlim(0, 40)
    ax.set_ylim(0, 25)
